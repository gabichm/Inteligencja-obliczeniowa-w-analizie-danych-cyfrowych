from aipython.stripsHeuristic import SimpleHeuristic


class HospitalCareHeuristic(SimpleHeuristic):
    def __init__(self, problem):
        # Initialize with the problem
        super().__init__(problem)

    def calculate(self, state):
        # Heuristic is based on the number of unsatisfied goals

        unsatisfied_goals = 0

        # Check if the goals related to patient 1 and 2 are satisfied
        if state.get('NeedsMedP1A') and not state.get('HasMedA'):
            unsatisfied_goals += 1
        if state.get('NeedsMedP1B') and not state.get('HasMedB'):
            unsatisfied_goals += 1
        if state.get('NeedsMedP1C') and not state.get('HasMedC'):
            unsatisfied_goals += 1
        if state.get('NeedsTestP1') and not state.get('HandsClean'):
            unsatisfied_goals += 1
        if state.get('CheckedChartP1') == False:
            unsatisfied_goals += 1
        if state.get('UpToDateChartP1') == False:
            unsatisfied_goals += 1

        if state.get('NeedsMedP2A') and not state.get('HasMedA'):
            unsatisfied_goals += 1
        if state.get('NeedsMedP2B') and not state.get('HasMedB'):
            unsatisfied_goals += 1
        if state.get('NeedsMedP2C') and not state.get('HasMedC'):
            unsatisfied_goals += 1
        if state.get('NeedsTestP2') and not state.get('HandsClean'):
            unsatisfied_goals += 1
        if state.get('CheckedChartP2') == False:
            unsatisfied_goals += 1
        if state.get('UpToDateChartP2') == False:
            unsatisfied_goals += 1

        # This is a very simple heuristic: it's just the number of unsatisfied goals
...
#
#
# def test_forward_heuristic(thisproblem=strips.simple_problem1):
...
#
# def test_regression_heuristic(thisproblem=strips.simple_problem1):
